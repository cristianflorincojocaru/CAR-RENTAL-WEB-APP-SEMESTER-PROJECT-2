...
export class BookingComponent implements OnInit {
  ...
  protections: Protection[] = [
    ...
    },
    {
      ...
      features: [
        ...
        'Zero excess on all damage',
        'Personal accident cover',
        '24/7 priority roadside',
      ],
      price: 16,
    },
  ];
  ...
  extras: Extra[] = [
    ...
    },
    {
      id: 'full_insurance',
      icon: '🛡️',
      name: 'Full Insurance / Zero Excess',
  ...
  get discountedBasePrice(): number {
    ...
    if (this.car.isOffer && this.car.discountPercent) {
      return Math.round(this.car.dailyRate * (1 - this.car.discountPercent / 100));
    }
    return this.car.dailyRate;
  }
  ...
  private calcDays(): void {
    ...
    const diff =
      new Date(this.form.returnDate).getTime() - new Date(this.form.pickupDate).getTime();
  ...
  private validateStep(): boolean {
    ...
    if (this.currentStep === 1) {
      ...
      if (!this.form.pickupLocation)
        this.formErrors.pickupLocation = 'Please select a pick-up location.';
      if (!this.form.sameReturn && !this.form.returnLocation) {
        this.formErrors.returnLocation = 'Please select a return location.';
      }
    }
  ...
  confirmBooking(): void {
    ...
    const request: CreateBookingRequest = {
      ...
      returnLocation: this.form.sameReturn ? this.form.pickupLocation : this.form.returnLocation,
      ...
      flightNumber: this.form.flightNumber || undefined,
      notes: this.form.notes || undefined,
      payNow: this.form.payNow,
    };

    this.bookingService.create(request).subscribe({
      next: (res) => {
        this.isLoading = false;
        this.bookingRef = res.bookingReference;
        this.currentStep = 4;
        window.scrollTo({ top: 0, behavior: 'smooth' });
      },
      error: (err: HttpErrorResponse) => {
        ...
        if (err.status === 409) {
          this.bookingApiError =
            'This vehicle is no longer available for the selected period. Please choose different dates.';
        } else if (err.status === 401) {
          this.router.navigate(['/login'], { queryParams: { returnUrl: this.router.url } });
        } else if (err.status === 0) {
          this.bookingApiError = 'Cannot connect to server. Please try again.';
        } else {
          this.bookingApiError = err.error?.detail || 'Booking failed. Please try again.';
        }
      },
    });
  }
